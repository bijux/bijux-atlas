...
| `ops.gate.binary-policy` | `governance` | `ops.root.validate` | forbid binary or unsupported artifact formats outside ops/**/assets |
...
| `ops.gate.control-plane-drift` | `docs` | `ops.root.validate` | ops/README.md must match control-plane snapshot contract or avoid hardcoded crate lists |
...
| `ops.gate.docs` | `docs` | `ops.root.validate` | block orphan docs, broken links, and stale contract lists |
| `ops.gate.docs-paths` | `docs` | `ops.root.validate` | docs markdown path links must resolve to existing targets |
| `ops.gate.evidence` | `report` | `ops.root.validate` | enforce curated evidence allowlist and bundle validity |
...
| `ops.gate.inventory` | `inventory` | `ops.root.validate` | enforce deterministic inventory registries and prevent semantic duplicates |
| `ops.gate.inventory-owner-fragments-live` | `inventory` | `ops.root.validate` | owner-docs.fragments.json entries must reference live owner docs |
...
| `ops.gate.inventory-registry-ordering` | `inventory` | `ops.root.validate` | generated inventory registries must use deterministic stable ordering |
| `ops.gate.inventory-registry-role` | `inventory` | `ops.root.validate` | inventory registry files must declare role as authored or generated |
| `ops.gate.inventory-registry-uniqueness` | `inventory` | `ops.root.validate` | forbid duplicate semantic registry files for the same inventory contract domain |
...
| `ops.gate.inventory-tool-registry-uniqueness` | `inventory` | `ops.root.validate` | tools.toml is the only tool probe registry; registry.toml is check catalog only |
| `ops.gate.k8s-determinism` | `k8s` | `ops.k8s.check` | k8s render/install determinism checks |
| `ops.gate.ledger` | `governance` | `ops.root.validate` | enforce ops ledger coverage, necessity reasons, and reference integrity |
...
| `ops.gate.no-stale-locked-claims` | `docs` | `ops.root.validate` | non-generated contract docs must not contain stale locked list claims |
...
| `ops.gate.orphan-report` | `governance` | `ops.root.validate` | orphan-files-report and file-usage-report must match live repository state |
...
| `ops.gate.ssot` | `governance` | `ops.root.validate` | ssot invariants: no duplicate authored truth, no obs domain, and no schemas outside ops/schema |
| `ops.gate.stack-reproducibility` | `stack` | `ops.stack.validate` | stack reproducibility and profile validation |
| `ops.gate.structure` | `governance` | `ops.root.validate` | enforce required files contracts and placeholder directory declarations |
