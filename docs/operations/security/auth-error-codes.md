...
- `AuthenticationRequired` (`401`): the request reached a protected route without valid
  credentials for the configured auth mode, or without the required trusted-boundary identity
  header for `oidc` or `mtls`.
...
- For `AuthenticationRequired`, provide the required API key or route the request through the
  configured ingress auth boundary so the required identity header is present for `oidc` or `mtls`.
