...
| SSOT contract drift | `scripts/areas/contracts/check_all.sh`, `api-contract-check`, `ci-api-contract` |
| OpenAPI drift + breaking change detection | `scripts/areas/public/openapi-diff-check.sh`, `scripts/areas/contracts/check_breaking_contract_change.py`, `ci-openapi-drift` |
...
| Policy relaxations registry (SSOT exceptions) | `configs/policy/policy-relaxations.json`, `scripts/areas/policy/find_relaxations.sh`, `xtask scan-relaxations`, `scripts/areas/public/policy-audit.py`, `ci-policy-relaxations` |
...
| SQLite index/schema contracts | `scripts/areas/contracts/check_sqlite_indexes_contract.py`, ingest schema/index drift tests, `ci-sqlite-*` |
