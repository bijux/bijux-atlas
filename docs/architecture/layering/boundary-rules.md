...
- `e2e`: The e2e layer consumes canonical `atlasctl ops ...` entrypoints (or thin make wrappers) and must never patch, mutate, or repair infrastructure directly.
- `obs`: The observability layer validates telemetry contracts and drill evidence but must not perform deployment fixups.
