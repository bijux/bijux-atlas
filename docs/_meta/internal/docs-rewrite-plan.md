...
8. Governance cluster (`docs/governance/**`, `docs/ownership.md`, `docs/style.md`) owner: `docs-governance`
9. Generated outputs cluster (`docs/_generated/**`) owner: `docs-governance`
...
3. Delete duplicated governance pages under `docs/_style/` after merge into `docs/governance/`.
...
- `docs/_generated/**` is frozen as output-only surface.
- Policy source: `docs/governance/generated-content-policy.md`.

## `_style` Consolidation Decision

- Decision: converge governance policy pages into `docs/governance/`.
- Keep `_style` only if content is uniquely implementation-specific; otherwise delete duplicates.

## Governance Placement Decision

- Decision: `docs/governance/` remains the single canonical location for docs policy controls.
- Existing governance pages outside `docs/governance/` must be merged or removed.

## ADR Placement Decision

- Move `docs/adrs/` content to `docs/governance/adrs/`.
- `docs/governance/adrs/` becomes the canonical decision-record directory.
