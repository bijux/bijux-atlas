# Site map

This documentation set is organized around three audience paths and a shared reader spine.

## Audience paths

- API user: Start here -> API -> Reference
- Operator: Start here -> Operations -> Reference
- Contributor: Start here -> Development -> Docs governance

## Core sections

- [Home](index.md)
- [Start here](start-here.md)
- [Product](product/index.md)
- [Architecture](architecture/index.md)
- [API](api/index.md)
- [Operations](operations/index.md)
- [Development](development/index.md)
- [Reference](reference/index.md)
- [Glossary](glossary.md)

## Navigation policy

- Section index pages are curated.
- Reader pages should be reachable in three clicks or fewer from their section index.
- Internal system pages (`_generated`, `_drafts`, `_nav`, `_meta`) are excluded from reader navigation.

## Next steps

- Reading tracks: [What to read next](what-to-read-next.md)
- Contributor docs governance: [Docs governance](governance/index.md)
