...
2. Human entrypoints are only `make <public-target>` and `ops/run/*.sh`.
3. Docs may reference only public make targets and `ops/run/*` scripts (allowlist exceptions only).
