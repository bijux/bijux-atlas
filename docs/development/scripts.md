...
- Public wrappers should stay thin; move reusable logic into Python modules (`packages/atlasctl/src`) or atlasctl-owned ops helper assets.
- Ops shared shell helpers are canonical in atlasctl-owned assets (for example `packages/atlasctl/src/atlasctl/commands/ops/runtime_modules/assets/lib/`).
