...
def _generate_configs_index(repo_root: Path) -> Path:
    ...
    lines = [
        "# Configs Index",
        "",
        "Generated by `bijux-atlas-scripts configs index`.",
        "",
        "| Path | Owner |",
        "|---|---|",
    ]
...
def run_configs_command(ctx: RunContext, ns: argparse.Namespace) -> int:
    ...
    if ns.configs_cmd == "inventory":
        ...
        if getattr(ns, "format", "json") == "md":
            ...
            result = {"schema_version": 1, "output": out.relative_to(repo).as_posix(), "tool": "bijux-atlas-scripts"}
        else:
            result = payload
    ...
    command_map: dict[str, Callable[[Path], tuple[int, str]]] = {
        ...
        "no-shadow-check": lambda r: _run(
            [
                ...
                "run",
                "./tools/bijux-atlas-scripts/src/bijux_atlas_scripts/layout/no_shadow.py",
            ],
            r,
        ),
        ...
        "directory-budgets-check": lambda r: _run(
            [
                "./scripts/bin/bijux-atlas-scripts",
                "run",
                "./tools/bijux-atlas-scripts/src/bijux_atlas_scripts/layout/dir_budgets.py",
            ],
            r,
        ),
