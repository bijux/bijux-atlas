...
contract-effect: ## Run effect contract execution through the canonical suite runner
	@$(DEV_ATLAS) suites run --suite contracts --jobs $(JOBS) --mode effect $(CONTRACT_FAIL_FAST_FLAG) --format $(FORMAT)

contract-all: ## Run the complete contract execution set through the canonical suite runner
	@$(DEV_ATLAS) suites run --suite contracts --jobs $(JOBS) --mode all $(CONTRACT_FAIL_FAST_FLAG) --format $(FORMAT)
