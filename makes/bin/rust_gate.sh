...
rs_nextest_cache_dir="${RS_NEXTEST_CACHE_DIR:-${NEXTEST_CACHE_DIR:-${rs_target_dir}/nextest}}"
...
rustfmt_config_path="${RUSTFMT_CONFIG_PATH:-configs/sources/repository/rust-tooling/rustfmt.toml}"
...
if [ -f "${audit_allowlist_path}" ]; then
  ...
  done < <(rg -o --no-line-number 'RUSTSEC-[0-9]{4}-[0-9]{4}' "${audit_allowlist_path}" | sort -u)
fi
...
case "${command_name}" in
  fmt)
    printf '%s\n' "run: cargo fmt --all -- --check --config-path ${rustfmt_config_path}"
    run_logged "${rs_fmt_report}" env \
      CARGO_TARGET_DIR="${rs_target_dir}" \
      cargo fmt --all -- --check --config-path "${rustfmt_config_path}"
    ;;
  lint)
  ...
  audit)
    ...
    audit_status=0
    {
      echo "run: cargo run -q -p bijux-atlas-dev -- security dependency-audit --repo-root ${workspace_root} --format json"
      CARGO_TARGET_DIR="${rs_target_dir}" cargo run -q -p bijux-atlas-dev -- security dependency-audit --repo-root "${workspace_root}" --format json || dependency_status=$?
      echo
