...
REGISTRY_CONTRACTS_TARGETS := contracts ssot-check policy-lint policy-schema-drift policy-audit policy-enforcement-status policy-allow-env-lint policy-drift-diff ops-policy-audit rename-lint config-validate config-print config-drift openapi-drift api-contract-check ops-values-validate ops-chart-render-diff ops-openapi-validate ops-dashboards-validate ops-alerts-validate
...
REGISTRY_CI_TARGETS := ci-root-layout ci-script-entrypoints ci-rename-lint ci-fmt ci-clippy ci-test-nextest ci-deny ci-audit ci-license-check ci-policy-lint ci-policy-schema-drift ci-policy-relaxations ci-policy-enforcement ci-policy-allow-env ci-ops-policy-audit ci-config-check ci-ssot-drift ci-crate-structure ci-crate-docs-contract ci-cli-command-surface ci-release-binaries ci-docs-build ci-latency-regression ci-store-conformance ci-store-conformance-localfs ci-store-conformance-http ci-store-conformance-s3 ci-openapi-drift ci-chart-schema-validate ci-api-contract ci-query-plan-gate ci-critical-query-check ci-sqlite-schema-drift ci-sqlite-index-drift ci-ingest-determinism ci-qc-fixtures ci-log-fields-contract ci-compatibility-matrix-validate ci-runtime-security-scan-image ci-coverage ci-workflows-make-only
