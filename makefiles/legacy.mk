...
legacy/root-local-full: ## DEPRECATED legacy local superset gate with 5 parallel isolated lanes + unified summary
	@PARALLEL=1 MODE=root-local RUN_ID="$${RUN_ID:-legacy-root-local-$(MAKE_RUN_TS)}" ./ops/run/root-lanes.sh

legacy/root-local-fast: ## DEPRECATED legacy local superset fast mode (skip stack-smoke lane)
	@PARALLEL=0 MODE=root RUN_ID="$${RUN_ID:-legacy-root-fast-$(MAKE_RUN_TS)}" ./ops/run/root-lanes.sh

legacy/ci: ## DEPRECATED legacy root + CI-only packaging/publish checks
	@$(call with_iso,ci,$(MAKE) -s root ci-release-binaries ci-docs-build ci-release-compat-matrix-verify)

legacy/nightly: ## DEPRECATED legacy nightly superset (ci + nightly ops suites)
	@$(call with_iso,nightly,$(MAKE) -s ci ops-load-nightly ops-drill-suite ops-realdata)

legacy/local-fast-loop: ## DEPRECATED legacy fast local loop
	@mkdir -p "$(LOCAL_ISO_ROOT)/target" "$(LOCAL_ISO_ROOT)/cargo-home" "$(LOCAL_ISO_ROOT)/tmp"
	@$(LOCAL_ENV) $(MAKE) fmt
	@$(LOCAL_ENV) $(MAKE) lint
	@$(LOCAL_ENV) $(MAKE) test

legacy/local-full-loop: ## DEPRECATED legacy full local loop
...
legacy/contracts: ## DEPRECATED legacy contracts meta pipeline
...
legacy/hygiene: ## DEPRECATED legacy repo hygiene checks
