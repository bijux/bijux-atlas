...
internal-list: ## Print internal make targets for maintainers
	@$(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/layout_checks/list_internal_targets.py
...
evidence/check: ## Validate evidence JSON schema contract for generated outputs
	@$(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/layout_checks/evidence_check.py
...
budgets/check: ## Validate universal budgets and budget-relaxation expiry policy
	@$(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/layout_checks/check_ops_budgets.py
...
perf/regression-check: ## Fail if p95 regression exceeds configured budget
	@PROFILE="$${PROFILE:-$${ATLAS_PERF_BASELINE_PROFILE:-local}}"; \
	$(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/load/regression_check.py --profile "$$PROFILE" --results "$${RESULTS:-artifacts/perf/results}"

perf/triage: ## Print top p95 regressions by suite from latest perf results
	@PROFILE="$${PROFILE:-$${ATLAS_PERF_BASELINE_PROFILE:-local}}"; \
	$(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/load/triage_regressions.py --profile "$$PROFILE" --results "$${RESULTS:-artifacts/perf/results}"
...
perf/compare: ## Compare two evidence perf runs (FROM=<run_id> TO=<run_id>)
	...
	@$(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/load/compare_runs.py --from-run "$${FROM}" --to-run "$${TO}"
...
cleanup/verify: ## One-time cleanup safety verification before deleting legacy paths
	...
	@$(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/layout_checks/check_help_snapshot.py && $(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/layout_checks/check_no_dead_entrypoints.py && $(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/layout_checks/check_no_orphan_docs_refs.py && $(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/layout_checks/check_no_orphan_configs.py && $(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/layout_checks/check_no_orphan_owners.py
...
root-determinism: ## Assert make root determinism (inventory outputs stable across two runs)
	@./packages/atlasctl/src/atlasctl/layout_checks/check_root_determinism.sh
...
artifacts-index: ## Generate artifacts index for inspection UIs
	@$(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/layout_checks/build_artifacts_index.py

artifacts-clean: ## Clean old artifacts with safe retention
	@$(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/layout_checks/clean_artifacts.py
...
clean-safe: ## Clean only safe generated make artifact directories
	@$(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/layout_checks/clean_make_artifacts.py
...
clean-all: ## Clean all allowed generated dirs (requires CONFIRM=YES)
	...
	@$(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/layout_checks/clean_make_artifacts.py --all
...
makefiles-contract: ## Validate makefile contract boundaries and publication rules
	@$(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/layout_checks/check_makefiles_contract.py

ci-workflow-contract: ## Validate CI and nightly workflows use canonical make entrypoints
	@$(ATLAS_SCRIPTS) run ./packages/atlasctl/src/atlasctl/layout_checks/check_ci_entrypoints.py
