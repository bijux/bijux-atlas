...
if ! ./scripts/fixtures/fetch-medium.sh; then
  if [ ! -d "fixtures/medium/data" ]; then
    echo "[demo] medium fixture download failed and fixtures/medium/data is missing" >&2
    exit 1
  fi
  echo "[demo] fetch failed but local fixtures/medium/data exists; continuing"
fi
