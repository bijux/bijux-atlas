...
# Inputs: scripts tree, scripts/areas/docs/ENTRYPOINTS.md, Makefile + makefiles/*.mk.
...
entrypoints = (AREAS / "docs/ENTRYPOINTS.md").read_text() if (AREAS / "docs/ENTRYPOINTS.md").exists() else ""
...
for p in files:
    ...
    parts = rel.split("/")
    top = parts[2] if len(parts) > 2 and parts[1] == "areas" else (parts[1] if len(parts) > 1 else "")
...
for d in sorted(p.name for p in AREAS.iterdir() if p.is_dir()):
    surface_lines.append(f"- `scripts/areas/{d}/`")
