...
if "./ops/k8s/scripts/deploy_atlas.sh" not in ops_mk:
    errors.append("ops-deploy must call ./ops/k8s/scripts/deploy_atlas.sh")
...
for path in [ROOT / "ops/e2e/realdata/run_single_release.sh", ROOT / "ops/e2e/realdata/run_two_release_diff.sh"]:
    ...
    if "/ops/e2e/scripts/deploy_atlas.sh" in txt:
        errors.append(f"{path.relative_to(ROOT)} must use ops/k8s/scripts/deploy_atlas.sh")
