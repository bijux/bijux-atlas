...
for code in error_codes:
    rust.append(f'            "{code}" => Some(Self::{code}),')
rust.extend(
    [
        "            _ => None,",
        "        }",
...
for metric in metrics:
    ...
    metrics_rs.append(f'    ("{metric["name"]}", &[{labels}]),')
metrics_rs.append("];\n")
...
for span in trace_spans:
    ...
    spans_rs.append(f'    ("{span["name"]}", &[{attrs}]),')
spans_rs.append("];\n")
