...
if contract_set != obs_set:
    print("METRICS.json drift from ops/obs/contract/metrics-contract.json", file=sys.stderr)
...
if sorted(obs_spans) != sorted(span_names):
    print("TRACE_SPANS.json drift from ops/obs/contract/metrics-contract.json", file=sys.stderr)
    sys.exit(1)
