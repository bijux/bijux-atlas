...
fn make_help_output_matches_golden() {
    ...
    let golden = fs::read_to_string(
        repo.join("crates/bijux-dev-atlas-core/tests/goldens/make_help.txt"),
    )
    .expect("read golden");
    assert_eq!(stdout.trim_end(), golden.trim_end(), "make help output drift");
}
...
fn root_curated_targets_are_documented_once() {
    ...
    assert!(targets.contains(&"clean".to_string()) == false, "clean is intentionally local alias not published yet");
}
