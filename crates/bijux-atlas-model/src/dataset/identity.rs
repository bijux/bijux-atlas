...
use bijux_atlas_core::canonical::{sha256_hex, stable_json_bytes};
...
impl DatasetIdentity {
    ...
    pub fn from_components(
        ...
        let source_bytes =
            stable_json_bytes(source_component).map_err(|err| ValidationError(err.to_string()))?;
        let build_bytes =
            stable_json_bytes(build_component).map_err(|err| ValidationError(err.to_string()))?;
        let artifact_bytes = stable_json_bytes(artifact_component)
...
pub fn canonical_identity_hash(
    ...
    let bytes = stable_json_bytes(&payload).map_err(|err| ValidationError(err.to_string()))?;
    Ok(sha256_hex(&bytes))
}
