...
use bijux_dev_atlas_model::RunReport;
use std::fs;
...
fn canonical_report_fixture_parses_and_matches_shape() {
    let fixture =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/run_report_canonical.json");
    let text = fs::read_to_string(fixture).expect("fixture");
    let parsed: RunReport = serde_json::from_str(&text).expect("parse");
    assert_eq!(
        parsed.schema_version,
        bijux_dev_atlas_model::schema_version()
    );
