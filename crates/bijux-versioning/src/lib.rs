...
pub fn latest_version_tag(workspace_root: &Path) -> Option<String> {
    let output = Command::new("git")
        .args(["-C", workspace_root.to_string_lossy().as_ref()])
...
pub fn version_from_tag(value: &str) -> Option<String> {
    normalize_version_string(value)
}

pub fn release_line_from_version(version: &str) -> Option<String> {
    let normalized = normalize_version_string(version)?;
    let parsed = Version::parse(&normalized).ok()?;
    Some(format!("v{}.{}", parsed.major, parsed.minor))
}
