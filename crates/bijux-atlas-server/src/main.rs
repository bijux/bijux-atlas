...
fn env_u64(name: &str, default: u64) -> Result<u64, String> {
    ...
    value
        .parse::<u64>()
        .map_err(|err| format!("invalid u64 value for {name}: {value} ({err})"))
}
...
fn env_usize(name: &str, default: usize) -> Result<usize, String> {
    ...
    value
        .parse::<usize>()
        .map_err(|err| format!("invalid usize value for {name}: {value} ({err})"))
}
...
fn env_f64(name: &str, default: f64) -> Result<f64, String> {
    ...
    value
        .parse::<f64>()
        .map_err(|err| format!("invalid f64 value for {name}: {value} ({err})"))
}
...
fn env_dataset_list(name: &str) -> Result<Vec<bijux_atlas_model::DatasetId>, String> {
    let Some(value) = env::var(name).ok() else {
        return Ok(Vec::new());
    };
    ...
    for item in value.split(',').map(str::trim).filter(|item| !item.is_empty()) {
        ...
        let dataset = bijux_atlas_model::DatasetId::new(parts[0], parts[1], parts[2])
            .map_err(|err| format!("invalid dataset list entry for {name}: {item} ({err})"))?;
        datasets.push(dataset);
    }
    Ok(datasets)
}

fn env_map(name: &str) -> Result<std::collections::HashMap<String, String>, String> {
    ...
    for item in raw.split(',').map(str::trim).filter(|item| !item.is_empty()) {
        ...
        if key.is_empty() || value.is_empty() {
            return Err(format!("invalid key=value entry for {name}: {item}"));
        }
...
async fn main() -> Result<(), String> {
    ...
    let effective_config_payload =
        bijux_atlas_server::effective_config_payload(&startup, &api_cfg, &cache_cfg)?;
    ...
    let shutdown_drain_ms = env_u64("ATLAS_SHUTDOWN_DRAIN_MS", 5000)?;
