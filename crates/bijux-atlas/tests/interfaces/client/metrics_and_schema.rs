...
use bijux_atlas::adapters::inbound::client::{ClientMetrics, InMemoryMetrics};
use reqwest as _;
use serde as _;
use serde_json as _;
use std::fs;
...
fn client_config_schema_is_parseable() {
    let path = repo_root()
        .join("configs/sources/runtime/clients/schemas/atlas-rust-client-config.schema.json");
