...
fn query_layer_must_not_depend_on_runtime_network_or_async_stacks() {
    let root = workspace_root();
    let query_src = root.join("crates/bijux-atlas/src/query");
    for forbidden in ["tokio", "reqwest", "axum", "hyper"] {
        for file in collect_rs_files(&query_src) {
            let content = fs::read_to_string(&file).expect("failed to read rust file");
            assert!(
                !content.contains(forbidden),
                "query layer must stay pure and cannot depend on runtime/net stack: {forbidden} in {}",
                file.display()
            );
        }
    }
...
fn ingestion_must_be_pure_transform_only() {
    let root = workspace_root();
    let ingest_src = root.join("crates/bijux-atlas/src/ingest");
    let forbidden = [
        "reqwest",
        "ureq",
        "hyper::",
