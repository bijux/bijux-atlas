...
pub(crate) fn build_release_diff(
    ...
    let to = DatasetId::new(&args.to_release, &args.species, &args.assembly)
        .map_err(|e| e.to_string())?;
    let from_paths = bijux_atlas_model::dataset::artifact_paths(&args.root, &from);
    let to_paths = bijux_atlas_model::dataset::artifact_paths(&args.root, &to);
...
fn index_by_identity(
    entries: &[bijux_atlas_query::ReleaseGeneIndexEntry],
) -> HashMap<String, bijux_atlas_query::ReleaseGeneIndexEntry> {
...
fn stable_gene_identity(entry: &bijux_atlas_query::ReleaseGeneIndexEntry) -> String {
