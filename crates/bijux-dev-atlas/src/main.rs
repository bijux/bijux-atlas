...
use crate::cli::{
    Cli, ConfigsCommand, ConfigsCommonArgs, DocsCommand, DocsCommonArgs, DomainArg, FormatArg, OpsCommand, OpsCommonArgs, OpsGenerateCommand,
...
fn configs_context(common: &ConfigsCommonArgs) -> Result<ConfigsContext, String> {
    ...
    let run_id = common.run_id.as_ref().map(|v| RunId::parse(v)).transpose()?.unwrap_or_else(|| RunId::from_seed("configs_run"));
    Ok(ConfigsContext {
...
fn parse_config_file(path: &Path) -> Result<(), String> {
...
fn configs_inventory_payload(ctx: &ConfigsContext, common: &ConfigsCommonArgs) -> Result<serde_json::Value, String> {
...
fn configs_validate_payload(ctx: &ConfigsContext, common: &ConfigsCommonArgs) -> Result<serde_json::Value, String> {
    let mut errors = Vec::<String>::new();
    let mut warnings = Vec::<String>::new();
    for required in ["configs/INDEX.md", "configs/README.md", "configs/contracts", "configs/schema"] {
        if !ctx.repo_root.join(required).exists() {
            errors.push(format!("CONFIGS_SCHEMA_ERROR: missing required config path `{required}`"));
        }
    }
    for file in configs_files(ctx) {
        let rel = file.strip_prefix(&ctx.repo_root).unwrap_or(&file).display().to_string();
...
fn configs_lint_payload(ctx: &ConfigsContext, common: &ConfigsCommonArgs) -> Result<serde_json::Value, String> {
    let mut errors = Vec::<String>::new();
    for file in configs_files(ctx) {
        let rel = file.strip_prefix(&ctx.repo_root).unwrap_or(&file).display().to_string();
        ...
        if rel.contains(' ') {
            errors.push(format!("CONFIGS_PARSE_ERROR: config path contains spaces `{rel}`"));
        }
        if name.chars().any(|c| c.is_ascii_uppercase()) {
            errors.push(format!("CONFIGS_PARSE_ERROR: config filename should be lowercase `{rel}`"));
        }
        let text = fs::read_to_string(&file).unwrap_or_default();
        for (i, line) in text.lines().enumerate() {
            if line.contains("${") {
                errors.push(format!("CONFIGS_PARSE_ERROR: {rel}:{} env interpolation defaults are forbidden", i + 1));
            }
    ...
    errors.sort();
    errors.dedup();
    Ok(serde_json::json!({"schema_version":1,"run_id":ctx.run_id.as_str(),"text": if errors.is_empty() {"configs lint passed"} else {"configs lint failed"},"errors":errors,"warnings":[],"capabilities":{"fs_write": common.allow_write, "subprocess": common.allow_subprocess, "network": common.allow_network},"options":{"strict": common.strict}}))
}

fn configs_compile_payload(ctx: &ConfigsContext, common: &ConfigsCommonArgs) -> Result<serde_json::Value, String> {
    if !common.allow_write {
        return Err("configs compile requires --allow-write".to_string());
    }
    let out_dir = ctx.artifacts_root.join("atlas-dev").join("configs").join(ctx.run_id.as_str());
    ...
    fs::write(&compiled_path, serde_json::to_string_pretty(&compiled).map_err(|e| e.to_string())?)
...
fn configs_diff_payload(ctx: &ConfigsContext, common: &ConfigsCommonArgs) -> Result<serde_json::Value, String> {
    ...
    let run = (|| -> Result<(String, i32), String> {
        ...
        match command {
            ...
            ConfigsCommand::Validate(common) => {
                ...
                if code != 0 && payload.get("error_code").is_none() { payload["error_code"] = serde_json::json!("CONFIGS_SCHEMA_ERROR"); }
...
mod tests {
    ...
    fn configs_subcommands_parse() {
        ...
        for argv in commands {
            let cli = super::Cli::try_parse_from(argv).expect("parse");
            match cli.command {
                Some(super::cli::Command::Configs { .. }) => {}
    ...
    fn parse_config_file_supports_json_yaml_toml() {
        ...
        fs::write(&yaml, "x: 1\n").expect("yaml");
        fs::write(&toml, "x = 1\n").expect("toml");
        assert!(super::parse_config_file(&json).is_ok());
        assert!(super::parse_config_file(&yaml).is_ok());
        assert!(super::parse_config_file(&toml).is_ok());
    }
}
