...
const PYTHON_PACKAGE_ROOT: &str = "packages/bijux-atlas-python";
...
fn collect_package_entries(repo_root: &Path) -> Result<Vec<String>, String> {
    ...
    let output = ProcessCommand::new("git")
        .args(["ls-files", "packages/bijux-atlas-python"])
...
fn run_packages_verify(args: &PackagesCommandArgs) -> Result<(String, i32), String> {
    ...
    let entries = collect_package_entries(&repo_root)?;
    let required_paths = [
        "packages/bijux-atlas-python/pyproject.toml",
        "packages/bijux-atlas-python/README.md",
        "packages/bijux-atlas-python/VERSIONING.md",
        "packages/bijux-atlas-python/compatibility.json",
    ];
    ...
    for path in entries {
        ...
        if path.ends_with(".py")
            && !path.starts_with("packages/bijux-atlas-python/src/")
        ...
        if path.ends_with(".ipynb") && !path.starts_with("packages/bijux-atlas-python/notebooks/") {
            violations.push(format!(
                "notebook file is outside approved package notebooks zone: {path}"
            ));
        }
        if path.starts_with("packages/bijux-atlas-python/tools/") {
            violations.push(format!(
