...
pub(crate) fn run_check_tree_budgets(
    ...
    let exceptions_path =
        repo_root.join("configs/sources/repository/repo-surface/tree-budget-exceptions.json");
    ...
    errors.extend(check_owner_coverage(
        &repo_root.join("configs/OWNERS.md"),
        "configs",
    )?);
    errors.extend(check_owner_coverage(
        &repo_root.join("docs/OWNERS.md"),
        "docs",
    )?);
    ...
    let command_index_path = repo_root.join("configs/generated/docs/command-index.json");
    ...
    let schema_index_path = repo_root.join("configs/generated/docs/schema-index.json");
    ...
    let command_ref_re = Regex::new(r"`bijux dev atlas ([a-z0-9_-]+(?: [a-z0-9_-]+)?)`")
        .map_err(|e| e.to_string())?;
    let make_ref_re = Regex::new(r"`make ([a-z0-9_-]+)`").map_err(|e| e.to_string())?;
    let path_ref_re =
        Regex::new(r"`((?:configs|ops/schema)/[a-zA-Z0-9_./-]+)`").map_err(|e| e.to_string())?;
...
pub(crate) fn run_check_repo_doctor(
    ...
    let tree_payload: serde_json::Value = serde_json::from_str(&tree_rendered)
        .map_err(|e| format!("tree payload parse failed: {e}"))?;
    ...
    let docs_code = if docs_payload["errors"]
        .as_array()
    ...
    let configs_code = if configs_payload["errors"]
        .as_array()
        .is_some_and(|v| !v.is_empty())
    {
        1
    ...
    let rendered = if explain && matches!(format, FormatArg::Text) {
        let mut lines = vec![payload["text"]
            .as_str()
            .unwrap_or("repo doctor")
            .to_string()];
        ...
        lines.push(
            "- surface_snapshot: validates committed root surface snapshot parity".to_string(),
        );
        let text = lines.join("\n");
