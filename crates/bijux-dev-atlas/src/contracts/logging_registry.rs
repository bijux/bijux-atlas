...
//! Canonical structured logging contract, classification rules, and format validation.
...
mod tests {
    ...
    #[test]
    fn log_schema_contract_has_expected_required_fields() {
        let contract = schema_contract();
        let required = contract
            .required_fields
            .iter()
    ...
    #[test]
    fn log_event_classification_maps_known_prefixes() {
        assert_eq!(classify_event_name("query_execute"), LogClass::Query);
        assert_eq!(classify_event_name("runtime_ready"), LogClass::Runtime);
        assert_eq!(
            classify_event_name("security_auth_denied"),
            LogClass::Security
        );
        assert_eq!(classify_event_name("misc_event"), LogClass::Unknown);
    }

    #[test]
    fn format_validator_flags_missing_fields_and_redaction_issues() {
        let row = serde_json::json!({
            "timestamp": "2026-03-05T10:00:00Z",
            "level": "INFO",
            "target": "atlas::runtime",
            "message": "api_token leaked",
            "event_name": "query_execute"
        });
        let violations = validate_log_record(&row);
        assert!(violations
            .iter()
            .any(|v| v.code == "missing_required_field"));
        assert!(violations.iter().any(|v| v.code == "redaction_violation"));
    }

    #[test]
    fn format_validator_accepts_well_formed_record() {
        let row = serde_json::json!({
            "timestamp": "2026-03-05T10:00:00Z",
            "level": "INFO",
            "target": "atlas::runtime",
