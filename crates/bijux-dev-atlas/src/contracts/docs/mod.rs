...
pub fn contract_explain(contract_id: &str) -> String {
    match contract_id {
        ...
        "DOC-008" => "Every top-level docs section directory must have a non-empty owner mapping in docs/_internal/registry/owners.json.".to_string(),
        "DOC-009" => "Every top-level docs section must be declared in docs/_internal/registry/sections.json, with no dead section entries.".to_string(),
        "DOC-010" => "Each top-level docs section must either require or forbid INDEX.md exactly as declared in docs/_internal/registry/sections.json.".to_string(),
        ...
        "DOC-016" => "Each required section INDEX.md must use the owner declared for that section in docs/_internal/registry/owners.json.".to_string(),
