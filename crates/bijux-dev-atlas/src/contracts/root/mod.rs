...
pub fn contract_explain(contract_id: &str) -> String {
    match contract_id {
        ...
        "ROOT-014" => "Generated workspace directories (`artifacts/`, `target/`, and `node_modules/`) must be ignored and never tracked.".to_string(),
        ...
        "ROOT-030" => "Vendor directories are forbidden across the repo except in the allowlisted `third_party/` subtree.".to_string(),
        ...
        "ROOT-037" => "Repository noise is forbidden: editor backups, `.DS_Store`, stray logs, tracked `target/`, `node_modules/`, and tracked executable (+x) files.".to_string(),
