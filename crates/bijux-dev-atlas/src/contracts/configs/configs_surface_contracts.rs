...
pub fn contracts(_repo_root: &Path) -> Result<Vec<Contract>, String> {
    Ok(vec![
        ...
        contract(
            "CONFIGS-044",
            "configs root authority files declare owners",
...
pub fn contract_explain(contract_id: &str) -> String {
    match contract_id {
        ...
        "CONFIGS-030" => "Every public or generated config file must have explicit file-level consumer coverage in configs/consumers-registry.json.".to_string(),
        "CONFIGS-031" => "Root, public, and generated JSON or JSONC configs must map to explicit schema coverage in configs/schema-map.json.".to_string(),
        ...
        "CONFIGS-033" => "The committed configs schema index must match the canonical render from configs/schema-map.json and the schema directories.".to_string(),
        "CONFIGS-034" => "Every input schema under configs/schema must be referenced by at least one governed config mapping in configs/schema-map.json.".to_string(),
