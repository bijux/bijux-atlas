...
#[cfg(test)]
pub(crate) fn parse_effect_for_test(raw: &str) -> Result<Effect, String> {
    parse_effect(raw)
}
...
#[cfg(test)]
pub(crate) fn registry_ordering_errors_for_test(registry: &Registry) -> Vec<String> {
    registry_ordering_errors(registry)
}
