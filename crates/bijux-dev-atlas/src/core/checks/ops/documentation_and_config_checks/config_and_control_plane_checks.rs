...
pub(super) fn check_make_configs_wrappers_delegate_dev_atlas(
    ...
    let rel = Path::new("make/makefiles/_configs.mk");
    ...
    if !content.contains("BIJUX ?= bijux") || !content.contains("BIJUX_DEV_ATLAS ?=") {
        violations.push(violation(
            ...
            "make/makefiles/_configs.mk must declare BIJUX and BIJUX_DEV_ATLAS variables"
                .to_string(),
    ...
    for line in content.lines().filter(|line| line.starts_with('\t')) {
        if line.trim_end().ends_with('\\') {
            violations.push(violation(
                ...
                "make/makefiles/_configs.mk wrapper recipes must be single-line delegations"
                    .to_string(),
        ...
        if words.iter().any(|w| {
            *w == "python"
            ...
            violations.push(violation(
                ...
                format!("make/makefiles/_configs.mk must remain delegation-only: `{line}`"),
    ...
    for required in ["configs-doctor:", "configs-validate:", "configs-lint:"] {
        if !content.contains(required) {
            violations.push(violation(
                ...
                format!("make/makefiles/_configs.mk is missing `{required}`"),
                "keep required configs delegation targets in make/makefiles/_configs.mk",
                Some(rel),
            ));
        }
    }
