...
pub fn governance_index_payload(repo_root: &Path, objects: &[GovernanceObject]) -> serde_json::Value {
        ...
        let covered_contracts = contracts
            ...
            .filter(|contract| {
                contract
                    .get("lanes")
                    .and_then(|v| v.as_array())
                    .is_some_and(|lanes| lanes.iter().any(|value| value.as_str() == Some(lane_name) || mapped_lane_name(value.as_str().unwrap_or_default()) == lane_name))
            })
            .filter_map(|contract| contract.get("contract_id").and_then(|v| v.as_str()).map(str::to_string))
            .collect::<Vec<_>>();
        rows.push(serde_json::json!({
            "lane": lane_name,
...
pub fn governance_orphan_checks_payload(repo_root: &Path) -> serde_json::Value {
    ...
    for lane in lanes {
        for command in lane
            ...
            .unwrap_or_default()
        {
            let id = command.get("id").and_then(|v| v.as_str()).unwrap_or_default();
            if id.contains('-')
                && id.chars().all(|ch| ch.is_ascii_uppercase() || ch.is_ascii_digit() || ch == '-')
...
pub fn governance_drift_payload(current_index: &serde_json::Value, previous_index: Option<&serde_json::Value>) -> serde_json::Value {
    ...
    let current_ids = current_contracts
        .iter()
        .filter_map(|row| row.get("contract_id").and_then(|v| v.as_str()).map(str::to_string))
        .collect::<BTreeSet<_>>();
    let previous_ids = previous_contracts
        .iter()
        .filter_map(|row| row.get("contract_id").and_then(|v| v.as_str()).map(str::to_string))
        .collect::<BTreeSet<_>>();
    let added = current_ids.difference(&previous_ids).cloned().collect::<Vec<_>>();
