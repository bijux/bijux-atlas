...
fn api_test_surface_stays_contract_owned() {
    ...
    for forbidden in ["bijux-atlas-runtime", "bijux-atlas-server", "axum =", "tokio ="] {
        assert!(
    ...
    for forbidden in ["tests/http_contracts.rs", "tests/http_surface.rs", "tests/http_observability.rs"] {
        assert!(
