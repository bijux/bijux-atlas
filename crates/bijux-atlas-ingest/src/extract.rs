    ...
    for rec in records {
        ...
        if rec.feature_type == "gene" {
            ...
            let Some(parent_attr) = rec.attrs.get("Parent") else {
                transcript_parents.push((
                    "<missing Parent attr>".to_string(),
                    ParentErrorClass::MissingParentAttribute,
                ));
                if matches!(opts.strictness, StrictnessMode::Strict) {
            ...
            if parents.len() > 1 {
                ...
                transcript_parents.push((p, ParentErrorClass::MissingReferencedParent));
            }
        }
    }
