...
use bijux_atlas_dev::docs::site_output::{
    validate_named_report, validate_report_value_against_schema,
};
use bijux_atlas_dev::engine::{
    ArtifactStore, CommandRunnableExecutor, EffectPolicy, RunStatus, RunnableExecutor,
    RunnableRunContext,
};
use bijux_atlas_dev::model::{RunId, RunnableId, RunnableKind, RunnableSelection};
use bijux_atlas_dev::registry::RunnableRegistry;
use bijux_atlas_dev::runtime::{Capabilities, RealFs, RealProcessRunner};
...
fn normalized_contract_command(command: &str) -> String {
    if matches!(
        command,
        "bijux dev atlas ops check" | "bijux-atlas-dev ops check"
    ) {
        "bijux-atlas-dev ops validate".to_string()
    } else if command.starts_with("bijux dev atlas contracts ops ")
        || command.starts_with("bijux-atlas-dev contracts ops ")
    {
        if command.contains("--allow-network") && command.contains("--allow-subprocess") {
            "bijux-atlas-dev ops validate --allow-subprocess --allow-network".to_string()
        } else if command.contains("--allow-network") {
            "bijux-atlas-dev ops validate --allow-network".to_string()
        } else if command.contains("--allow-subprocess") {
            "bijux-atlas-dev ops validate --allow-subprocess".to_string()
        } else {
            "bijux-atlas-dev ops validate".to_string()
        }
    } else {
    ...
    match report_id {
        ...
        "check-config-format" => {
            ...
            serde_json::json!({
                ...
                "summary": task.summary,
                "status": status,
                "tool_versions": tool_version_rows(repo_root, &["bijux-atlas-dev"]),
...
fn normalized_contract_runner_from_entry(
    entry: &bijux_atlas_dev::model::RunnableEntry,
) -> Vec<String> {
    entry
        .commands
        .iter()
...
pub(crate) fn run_suites_command(quiet: bool, command: SuitesCommand) -> i32 {
    ...
    match result {
        ...
        Err(err) => {
            let _ = writeln!(std::io::stderr(), "bijux-atlas-dev suites failed: {err}");
            1
        }
    }
}
