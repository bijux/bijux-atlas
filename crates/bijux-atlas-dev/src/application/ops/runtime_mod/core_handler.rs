    ...
    match command {
        ...
        OpsCommand::Doctor(common) => {
            ...
            let mut inventory_errors =
                match bijux_atlas_dev::core::ops_inventory::OpsInventory::load_and_validate(
                    &ops_root,
                ) {
                    Ok(_) => Vec::new(),
                    Err(err) => vec![err],
        ...
        OpsCommand::Package(args) => {
            ...
            let payload = serde_json::json!({
                ...
                "next_steps": [
                    "run `bijux-atlas-dev release ops validate-package --format json`",
                    "run `bijux-atlas-dev release ops readiness-summary --format json`"
                ]
            });
