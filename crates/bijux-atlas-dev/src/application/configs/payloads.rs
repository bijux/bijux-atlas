...
pub(crate) fn configs_validate_payload(
    ...
    if ci_lanes_path.exists() {
        ...
        for lane in lanes {
            ...
            if !workflow_text.contains("cargo run -q -p bijux-atlas-dev")
                && !workflow_text.contains("cargo run --locked -q -p bijux-atlas-dev")
        ...
        .filter(|p| p.extension().and_then(|v| v.to_str()) == Some("yml"))
    {
        ...
        for (line_idx, line) in content.lines().enumerate() {
            ...
            if line.contains("cargo fmt")
                ...
                && !line.contains("cargo run -q -p bijux-atlas-dev")
            {
                errors.push(format!(
