...
use bijux_atlas_dev::core::load_harness::{
...
fn write_supporting_run_artifacts(
    ...
    write_json(
        ...
        &serde_json::json!({
            ...
            "required_commands": [
                "bijux-atlas-dev load baseline --format json",
                "bijux-atlas-dev load run --format json",
                "bijux-atlas-dev load compare --format json"
            ],
            "failure_exit_code": 2
        }),
    )?;
