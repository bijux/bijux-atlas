...
use bijux_atlas_dev::domains::tutorials::checks;
use bijux_atlas_dev::domains::tutorials::runtime::workspace::TutorialWorkspaceManager;
use bijux_atlas_dev::ui::terminal::report::{render_status_line, LineStyle};
use std::fs;
...
pub(crate) fn run_tutorials_command(quiet: bool, command: TutorialsCommand) -> i32 {
    ...
    match run {
        ...
        Err(err) => {
            let _ = writeln!(io::stderr(), "bijux-atlas-dev tutorials failed: {err}");
            1
        }
    }
}
...
fn run_tutorials_explain(args: &TutorialsCommandArgs) -> Result<(String, i32), String> {
    ...
    let payload = serde_json::json!({
        ...
        "repo_root": repo_root.display().to_string(),
        "doctrine": {
            "automation_owner": "bijux-atlas-dev",
...
fn render_tutorial_explain_markdown() -> String {
    [
        ...
        "",
        "- Automation engine: `bijux-atlas-dev`",
        "- Primary workflow: `bijux-atlas-dev tutorials run workflow`",
        "- Validation entrypoint: `bijux-atlas-dev tutorials verify`",
    ]
    .join("\n")
}
...
fn run_tutorials_real_data_export_evidence(
    ...
    let evidence = serde_json::json!({
        ...
        "run_id": run.id,
        "dataset": run.dataset,
        "atlas_version": bijux_atlas_dev::version::runtime_version(),
        "git_sha": git_sha,
        ...
        "tool_versions": {
            "cargo": bijux_atlas_dev::version::runtime_version(),
            "git": "detected",
            "curl": "detected"
        },
