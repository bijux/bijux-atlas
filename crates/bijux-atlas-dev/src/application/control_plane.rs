...
use crate::*;
use bijux_atlas_dev::model::CONTRACT_SCHEMA_VERSION;
use bijux_atlas_dev::policies::{canonical_policy_json, DevAtlasPolicySet};
...
pub(crate) fn run_policies_command(quiet: bool, command: PoliciesCommand) -> i32 {
    ...
    match result {
        ...
        Err(err) => {
            let _ = writeln!(io::stderr(), "bijux-atlas-dev policies failed: {err}");
            1
        }
    }
}
...
pub(crate) fn run_version_command(
    ...
    let payload = serde_json::json!({
        ...
        "version": bijux_atlas_dev::version::runtime_version(),
        "semver": bijux_atlas_dev::version::runtime_semver(),
        "source": bijux_atlas_dev::version::runtime_version_source(),
    ...
    let rendered = match format {
        FormatArg::Text => {
            ...
            let git_hash = payload
                ...
                .and_then(|v| v.as_str())
                .unwrap_or("unknown");
            format!("bijux-atlas-dev {version}\ngit_hash: {git_hash}")
        }
