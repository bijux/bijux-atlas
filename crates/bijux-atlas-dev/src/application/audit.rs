...
fn bundle_generate(
    ...
    let bundle = serde_json::json!({
        ...
        "metadata": {
            "generator": "bijux-atlas-dev audit bundle generate",
...
fn audit_explain(common: crate::cli::AuditBundleArgs) -> Result<(String, i32), String> {
    ...
    let payload = serde_json::json!({
        ...
        "commands": [
            "bijux-atlas-dev audit run --format json",
            "bijux-atlas-dev audit report --format json",
            "bijux-atlas-dev audit explain --format json"
        ]
    });
    let rendered = emit_payload(common.format, common.out, &payload)?;
    Ok((rendered, 0))
