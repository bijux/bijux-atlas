...
fn run_bijux_atlas_dev_help(repo_root: &std::path::Path, args: &[&str]) -> Result<String, String> {
    let output = std::process::Command::new("cargo")
        .current_dir(repo_root)
        .args(["run", "-q", "-p", "bijux-atlas-dev", "--"])
        .args(args)
        .output()
...
fn render_docs_reference_commands(repo_root: &std::path::Path) -> Result<String, String> {
    let root_help =
        trim_help_usage_and_commands(&run_bijux_atlas_dev_help(repo_root, &["--help"])?);
    let ops_help =
        trim_help_usage_and_commands(&run_bijux_atlas_dev_help(repo_root, &["ops", "--help"])?);
    Ok(wrap_generated_reference_page(
...
fn render_docs_reference_ops_surface(repo_root: &std::path::Path) -> Result<String, String> {
    ...
    let mut entrypoints = value["entrypoints"]
        ...
        .collect::<Vec<_>>();
    entrypoints.sort();
    let mut commands = value["bijux-atlas-dev_commands"]
        .as_array()
        .cloned()
        .unwrap_or_default()
        .into_iter()
