...
pub(crate) fn run_makes_command(quiet: bool, command: MakesCommand) -> i32 {
    ...
    match run {
        ...
        Err(err) => {
            let _ = writeln!(io::stderr(), "bijux-atlas-dev makes failed: {err}");
            1
        }
    }
}
...
fn run_wrappers_verify(
    ...
    let payload = serde_json::json!({
        ...
        "verification_command": "cargo run -p bijux-atlas-dev -- check run --suite make_required --include-internal --include-slow --format json",
        "duration_ms": started.elapsed().as_millis() as u64,
        "exit_code": code,
    });
    let rendered = emit_payload(common.format, common.out.clone(), &payload)?;
