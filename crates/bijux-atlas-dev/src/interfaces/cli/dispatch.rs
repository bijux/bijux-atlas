...
use bijux_atlas_dev::domains;
use bijux_atlas_dev::model::exit_codes::{EXIT_FAILURE, EXIT_NOT_FOUND, EXIT_SUCCESS};
use bijux_atlas_dev::model::{RunnableId, RunnableKind, RunnableMode};
use bijux_atlas_dev::registry::{ReportRegistry, RunnableRegistry};
use bijux_atlas_dev::runtime::cli_adapter::{
...
fn run_check_surface_command(quiet: bool, command: CheckCommand) -> i32 {
    match command {
        CheckCommand::List {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev check list failed: {err}");
                1
            }
        },
        CheckCommand::Explain {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev check explain failed: {err}");
                1
            }
        },
        CheckCommand::Run {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev check run failed: {err}");
                1
            }
        },
        CheckCommand::Doctor {
            ...
            match crate::run_check_doctor(repo_root, include_internal, include_slow, format, out) {
                ...
                Err(err) => {
                    let _ = writeln!(io::stderr(), "bijux-atlas-dev check doctor failed: {err}");
                    1
                }
            }
        }
...
pub(crate) fn run_cli(cli: Cli) -> i32 {
    ...
    if cli.print_policies {
        return match run_print_policies(cli.repo_root.clone()) {
            ...
            Err(err) => {
                let _ = writeln!(
                    io::stderr(),
                    "bijux-atlas-dev --print-policies failed: {err}"
                );
                1
            }
        };
    ...
    if cli.print_boundaries {
        return match run_print_boundaries_command() {
            ...
            Err(err) => {
                let _ = writeln!(
                    io::stderr(),
                    "bijux-atlas-dev --print-boundaries failed: {err}"
                );
                1
            }
        };
    ...
    let Some(command) = cli.command else {
        let _ = writeln!(
            io::stderr(),
            "bijux-atlas-dev requires a subcommand unless --print-policies or --print-boundaries is provided"
        );
        return 2;
    };
    ...
    let exit = match command {
        Command::Version { format, out } => match run_version_command(format, out) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev version failed: {err}");
                1
            }
        },
        Command::Help { format, out } => match run_help_inventory_command(format, out) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev help failed: {err}");
                1
            }
        },
        ...
        Command::Governance { command } => match run_governance_command(cli.quiet, command) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev governance failed: {err}");
                1
            }
        },
        Command::Security { command } => match run_security_command(cli.quiet, command) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev security failed: {err}");
                1
            }
        },
        ...
        Command::Runtime { command } => match run_runtime_command(cli.quiet, command) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev runtime failed: {err}");
                1
            }
        },
        ...
        Command::System { command } => match run_system_command(cli.quiet, command) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev system failed: {err}");
                1
            }
        },
        Command::Audit { command } => match run_audit_command(cli.quiet, command) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev audit failed: {err}");
                1
            }
        },
        Command::Observe { command } => match run_observe_command(cli.quiet, command) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev observe failed: {err}");
                1
            }
        },
        Command::Api { command } => match run_api_command(cli.quiet, command) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev api failed: {err}");
                1
            }
        },
        Command::Load { command } => match run_load_command(cli.quiet, command) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev load failed: {err}");
                1
            }
        },
        Command::Invariants { command } => match run_invariants_command(cli.quiet, command) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev invariants failed: {err}");
                1
            }
        },
        Command::Drift { command } => match run_drift_command(cli.quiet, command) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev drift failed: {err}");
                1
            }
        },
        Command::Reproduce { command } => match run_reproduce_command(cli.quiet, command) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev reproduce failed: {err}");
                1
            }
        },
        Command::Datasets { command } => match run_data_command(
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev datasets failed: {err}");
                1
            }
        },
        Command::Ingest { command } => match run_data_command(
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev ingest failed: {err}");
                1
            }
        },
        Command::Perf { command } => match run_perf_command(cli.quiet, command) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev perf failed: {err}");
                1
            }
        },
        ...
        Command::Capabilities { format, out } => match run_capabilities_command(format, out) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev capabilities failed: {err}");
                1
            }
        },
        Command::Validate {
            ...
            let exe = match std::env::current_exe() {
                Ok(path) => path,
                Err(err) => {
                    let _ = writeln!(io::stderr(), "bijux-atlas-dev validate failed: {err}");
                    return 1;
                }
            };
        ...
        Command::Release { command } => match run_release_command(cli.quiet, command) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev release failed: {err}");
                1
            }
        },
        ...
        Command::Tests { command } => {
            ...
            match result {
                ...
                Err(err) => {
                    let _ = writeln!(io::stderr(), "bijux-atlas-dev tests failed: {err}");
                    1
                }
            }
        }
        Command::Reports { command } => match run_reports_command(cli.output_format, command) {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev reports failed: {err}");
                EXIT_FAILURE
            }
        },
        Command::List {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev list failed: {err}");
                EXIT_FAILURE
            }
        },
        Command::Describe {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev describe failed: {err}");
                EXIT_FAILURE
            }
        },
        Command::Run {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev run failed: {err}");
                EXIT_FAILURE
            }
        },
    ...
    if cli.verbose {
        let _ = writeln!(io::stderr(), "bijux-atlas-dev exit={exit}");
    }
    exit
}
