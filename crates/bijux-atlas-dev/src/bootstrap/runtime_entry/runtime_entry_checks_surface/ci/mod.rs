...
fn run_ci_verify_gate(
    ...
    let payload = match gate {
        ...
        "release-candidate" => {
            ...
            if metadata.status.success() {
                ...
                package_version = value["packages"]
                    ...
                    .into_iter()
                    .flatten()
                    .find(|pkg| pkg["name"].as_str() == Some("bijux-atlas-dev"))
            ...
            if let Some(tag) = git_ref.strip_prefix("refs/tags/") {
                if format!("v{package_version}") != tag {
                    errors.push(format!(
                        "tag `{tag}` does not match bijux-atlas-dev version `v{package_version}`"
                    ));
                }
            }
            serde_json::json!({
        ...
        "docs-preview" => {
            ...
            let site_payload = bijux_atlas_dev::docs::site_output::site_output_report(repo_root)?;
            serde_json::json!({
                "schema_version": 1,
    ...
    match command {
        WorkflowsCommand::Lanes { command } => match command {
                ...
                .and_then(|root| render_ci_lanes_list(&root, format, out))
            {
                ...
                Err(err) => {
                    let _ = writeln!(io::stderr(), "bijux-atlas-dev ci lanes list failed: {err}");
                    1
                }
            },
                ...
                .and_then(|root| render_ci_lanes_explain(&root, &lane_id, format, out))
            {
                ...
                Err(err) => {
                    let _ = writeln!(
                        io::stderr(),
                        "bijux-atlas-dev ci lanes explain failed: {err}"
                    );
                    1
                }
            },
                ...
                .and_then(|root| render_ci_lanes_validate(&root, format, out))
            {
                ...
                Err(err) => {
                    let _ = writeln!(
                        io::stderr(),
                        "bijux-atlas-dev ci lanes validate failed: {err}"
                    );
                    1
                }
            },
            ...
            .and_then(|root| render_ci_simulate(&root, lane, matrix, format, out))
        {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev ci simulate failed: {err}");
                1
            }
        },
        WorkflowsCommand::EnvContract { command } => match command {
                ...
                .and_then(|root| render_ci_env_contract_validate(&root, format, out))
            {
                ...
                Err(err) => {
                    let _ = writeln!(
                        io::stderr(),
                        "bijux-atlas-dev ci env-contract validate failed: {err}"
                    );
                    1
                }
            },
        ...
        WorkflowsCommand::Validate {
            ...
            Err(err) => {
                let _ = writeln!(
                    io::stderr(),
                    "bijux-atlas-dev workflows validate failed: {err}"
                );
                1
            }
        },
        WorkflowsCommand::Doctor {
            ...
            Err(err) => {
                let _ = writeln!(
                    io::stderr(),
                    "bijux-atlas-dev workflows doctor failed: {err}"
                );
                1
            }
        },
        WorkflowsCommand::Surface {
            ...
            Err(err) => {
                let _ = writeln!(
                    io::stderr(),
                    "bijux-atlas-dev workflows surface failed: {err}"
                );
                1
            }
        },
            ...
            .and_then(|root| render_ci_explain(&root, &lane, format, out))
        {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev ci explain failed: {err}");
                1
            }
        },
            ...
            .and_then(|root| render_ci_report(&root, &kind, format, out))
        {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev ci report failed: {err}");
                1
            }
        },
        WorkflowsCommand::Verify {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev ci verify failed: {err}");
                1
            }
        },
    }
...
pub(super) fn run_gates_command(quiet: bool, command: GatesCommand) -> i32 {
    match command {
        GatesCommand::List {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev gates list failed: {err}");
                1
            }
        },
        GatesCommand::Run {
            ...
            Err(err) => {
                let _ = writeln!(io::stderr(), "bijux-atlas-dev gates run failed: {err}");
                1
            }
        },
    }
