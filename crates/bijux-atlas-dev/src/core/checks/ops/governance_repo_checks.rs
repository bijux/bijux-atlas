    ...
    for rel in ["scripts", "tools", "xtask"] {
        ...
        if ctx.adapters.fs.exists(ctx.repo_root, path) {
            violations.push(violation(
                ...
                "delete the directory and move behavior into crates/bijux-atlas-dev command surfaces",
                Some(path),
            ));
        }
    }
...
pub(super) fn check_root_makefile_single_include_entrypoint(
    ...
    let mut lines = text
        ...
        .collect::<Vec<_>>();
    if lines.len() == 1 && lines[0] == "include makes/root.mk" {
        return Ok(Vec::new());
    }
...
pub(super) fn check_root_top_level_directories_contract(
    ...
    let expected = ["artifacts", "configs", "crates", "docs", "makes", "ops"];
...
pub(super) fn check_crates_bijux_atlas_owns_umbrella_dispatch(
    ctx: &CheckContext<'_>,
) -> Result<Vec<Violation>, CheckError> {
    ...
    for file in walk_files(&crates_root) {
        ...
        if rel.starts_with("crates/bijux-atlas-dev/")
            || rel == Path::new("crates/bijux-atlas-dev/src/core/checks/ops.rs")
            || rel.starts_with("crates/bijux-atlas-dev/src/core/checks/ops/")
        {
            continue;
        }
...
pub(super) fn check_crates_bijux_atlas_dev_help_dispatch_present(
    ctx: &CheckContext<'_>,
) -> Result<Vec<Violation>, CheckError> {
    let src = ctx
        .repo_root
