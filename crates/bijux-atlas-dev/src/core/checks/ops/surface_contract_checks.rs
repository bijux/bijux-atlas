    ...
    for target in expected_targets {
        if !content.contains(target) {
            violations.push(violation(
                ...
                "add thin ops wrapper target in makes/ops.mk",
                Some(rel),
            ));
        }
    }
    ...
    for line in content.lines().filter(|line| line.starts_with('\t')) {
        ...
        if words.iter().any(|w| {
            ...
            violations.push(violation(
                ...
                "keep governance wrappers routed only through makes/bijux dev atlas",
                Some(rel),
            ));
        }
    }
...
pub(super) fn check_make_ops_wrappers_delegate_dev_atlas(
    ...
    if !content.contains("BIJUX ?= bijux") || !content.contains("BIJUX_DEV_ATLAS ?=") {
        violations.push(violation(
            ...
            "makes/ops.mk must declare BIJUX and BIJUX_DEV_ATLAS variables".to_string(),
            "declare BIJUX and BIJUX_DEV_ATLAS wrapper variables in makes/ops.mk",
            Some(rel),
        ));
    }
    for line in content.lines().filter(|line| line.starts_with('\t')) {
        if line.trim_end().ends_with('\\') {
            violations.push(violation(
                ...
                "makes/ops.mk wrapper recipes must be single-line delegations".to_string(),
        ...
        if matches!(
            ...
            violations.push(violation(
                ...
                format!("makes/ops.mk must be delegation-only: `{line}`"),
...
pub(super) fn check_workflows_policy_registry_unique_and_documented(
    ...
    for entry in value["entries"].as_array().into_iter().flatten() {
        ...
        if docs.trim().is_empty() || !ctx.repo_root.join(docs).exists() {
            ...
            if !doc_text.contains("bijux-atlas-dev --")
                && !doc_text.contains("cargo run -q -p bijux-atlas-dev --")
                && !doc_text.contains("cargo run --locked -q -p bijux-atlas-dev --")
            {
                violations.push(violation(
...
pub(super) fn check_docs_command_list_matches_contract(
    ...
    let rel = Path::new("docs/bijux-atlas-crate/cli-command-list.md");
...
pub(super) fn check_docs_dev_command_list_matches_contract(
    ...
    let rel = Path::new("docs/bijux-atlas-dev-docs/cli-command-list.md");
    let current = fs::read_to_string(ctx.repo_root.join(rel))
        .map_err(|err| CheckError::Failed(err.to_string()))?;
...
pub(super) fn check_crates_bijux_atlas_dev_not_umbrella_binary(
    ctx: &CheckContext<'_>,
) -> Result<Vec<Violation>, CheckError> {
    let rel = Path::new("crates/bijux-atlas-dev/Cargo.toml");
    let text = fs::read_to_string(ctx.repo_root.join(rel))
        .map_err(|err| CheckError::Failed(err.to_string()))?;
    if text.contains("name = \"bijux\"") {
        Ok(vec![violation(
            "CRATES_DEV_ATLAS_UMBRELLA_BINARY_FORBIDDEN",
            "bijux-atlas-dev must not build the umbrella `bijux` binary".to_string(),
...
pub(super) fn check_crates_command_namespace_ownership_unique(
    ...
    let runtime_rel = Path::new("docs/bijux-atlas-crate/cli-command-list.md");
    let dev_rel = Path::new("docs/bijux-atlas-dev-docs/cli-command-list.md");
