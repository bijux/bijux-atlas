...
const LEGACY_ATLAS_DEV_SPLIT_CRATE_NAMES: &[&str] = &[
    "bijux-atlas-dev-core",
    "bijux-atlas-dev-model",
    "bijux-atlas-dev-adapters",
    "bijux-atlas-dev-policies",
];
...
fn allowlisted_paths() -> BTreeSet<&'static str> {
    ["crates/bijux-atlas-dev/tests/legacy_atlas_dev_split_crate_names.rs"]
        .into_iter()
        .collect()
}
...
fn assert_no_legacy_atlas_dev_split_crate_names_outside_guard(legacy_names: &[&str]) {
    ...
    assert!(
        violations.is_empty(),
        "unexpected legacy atlas-dev split crate references remain: {violations:?}"
    );
}

#[test]
fn no_unexpected_legacy_atlas_dev_split_crate_names_remain() {
    assert_eq!(
        LEGACY_ATLAS_DEV_SPLIT_CRATE_NAMES.len(),
        4,
        "the legacy atlas-dev split crate-name guard must list the four removed split crate names"
    );
    assert_no_legacy_atlas_dev_split_crate_names_outside_guard(LEGACY_ATLAS_DEV_SPLIT_CRATE_NAMES);
}

#[test]
fn duplicate_migration_surface_roots_are_explicit_and_do_not_expand() {
    let src_root = workspace_root().join("crates/bijux-atlas-dev/src");
    ...
    assert_eq!(
        ...
        "duplicate source surface roots changed; update the explicit overlap contract intentionally"
    );
}
