...
fn no_unwrap_or_expect_in_non_test_dev_atlas_sources() {
    let root = repo_root().join("crates/bijux-atlas-dev/src");
    let allowed_inline_test_files: BTreeSet<&str> = [
        "crates/bijux-atlas-dev/src/adapters/mod.rs",
        ...
        "crates/bijux-atlas-dev/src/application/ops/workspace_contracts.rs",
...
fn no_todo_macro_anywhere_in_dev_atlas_crate() {
    ...
    let output = Command::new("git")
        ...
        .args([
            ...
            r"\btodo!\(",
            "--",
            ":(glob)crates/bijux-atlas-dev/**/*.rs",
            ":(glob)crates/bijux-atlas-dev/*.rs",
        ])
        .output()
        .expect("run git grep");
