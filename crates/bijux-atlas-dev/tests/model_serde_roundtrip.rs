...
fn fingerprint_is_stable_for_same_violation() {
    ...
    let one = bijux_atlas_dev::model::fingerprint::violation_fingerprint(violation);
    let two = bijux_atlas_dev::model::fingerprint::violation_fingerprint(violation);
    assert_eq!(one, two);
}
