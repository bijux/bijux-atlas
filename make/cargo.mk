...
lint-policy-enforce: ## Enforce repository lint drift guards
	@! rg -n '\btodo!\(' crates
	@! rg -n '\bdbg!\(' crates
	@! rg -n '\b(?:println|eprintln)!\(' crates/bijux-atlas/src crates/bijux-atlas-server/src crates/bijux-atlas-core/src crates/bijux-atlas-model/src crates/bijux-atlas-store/src crates/bijux-atlas-ingest/src crates/bijux-atlas-api/src crates/bijux-dev-atlas/src --glob '!**/tests/**' --glob '!**/benches/**' --glob '!**/main.rs' --glob '!**/bin/**'
	@! rg -n '\bpanic!\(' crates/bijux-atlas crates/bijux-atlas-core crates/bijux-atlas-model crates/bijux-atlas-ingest crates/bijux-atlas-store crates/bijux-atlas-api --glob '!**/tests/**' --glob '!**/benches/**'
...
test-slow: ## Run only slow_ tests with cargo nextest
	...
	cleanup() { $(cleanup_root_nextest); }; trap cleanup EXIT INT TERM; \
	CARGO_TERM_COLOR=$(CARGO_TERM_COLOR) CARGO_TERM_PROGRESS_WHEN=$(CARGO_TERM_PROGRESS_WHEN) CARGO_TERM_PROGRESS_WIDTH=$(CARGO_TERM_PROGRESS_WIDTH) CARGO_TERM_VERBOSE=$(CARGO_TERM_VERBOSE) NEXTEST_CACHE_DIR="$(NEXTEST_CACHE_DIR)" cargo nextest run --color always --cargo-quiet --workspace --config-file configs/nextest/nextest.toml --target-dir "$(CARGO_TARGET_DIR)" --profile "$${NEXTEST_PROFILE:-default}" --status-level "$${NEXTEST_STATUS_LEVEL:-all}" --final-status-level "$${NEXTEST_FINAL_STATUS_LEVEL:-all}" -E "test(/(^|::)slow_/)" 2>&1 | tee "$$report_file"; \
	status=$${PIPESTATUS:-$${pipestatus}}; \
	$(nextest_summary); \
	trap - EXIT INT TERM; cleanup; \
	test $$status -eq 0
