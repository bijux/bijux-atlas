...
| `ops/inventory/pins.yaml` | `ops/schema/meta/pins.schema.json` (semantic mirror) | `bijux dev atlas ops pins update` (compat output still JSON) | Canonical migration target |
| `ops/inventory/toolchain.yaml` | `ops/schema/stack/version-manifest.schema.json` + tool-versions SSOT | `bijux dev atlas ops stack versions-sync` + migration tooling | Unified toolchain inventory |
