...
ART_DIR="$ROOT/artifacts/ops/observability/tests"
mkdir -p "$ART_DIR"
...
capture_failure_artifacts() {
  ...
  cp -f "$ROOT/ops/observability/grafana/atlas-observability-dashboard.json" "$out/dashboard.json" 2>/dev/null || true
  cp -f "$ROOT/ops/observability/alerts/atlas-alert-rules.yaml" "$out/alerts.yaml" 2>/dev/null || true
  echo "$out"
}
