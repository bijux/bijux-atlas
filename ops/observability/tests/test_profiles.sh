...
if ! kubectl api-resources | grep -q '^servicemonitors'; then
  if ATLAS_OBS_PROFILE=cluster "$ROOT/ops/observability/scripts/install_pack.sh" >/dev/null 2>&1; then
    echo "cluster profile unexpectedly succeeded without ServiceMonitor CRD" >&2
    exit 1
  fi
else
  ATLAS_OBS_PROFILE=cluster "$ROOT/ops/observability/scripts/install_pack.sh"
  "$ROOT/ops/observability/scripts/uninstall_pack.sh"
fi
