...
python3 "$ROOT/ops/observability/scripts/contracts/check_metrics_contract.py"
python3 "$ROOT/ops/observability/scripts/contracts/check_metrics_coverage.py"
python3 "$ROOT/ops/observability/scripts/contracts/check_metrics_drift.py"
python3 "$ROOT/ops/observability/scripts/contracts/check_metrics_golden.py"
python3 "$ROOT/ops/observability/scripts/contracts/check_tracing_contract.py"
python3 "$ROOT/ops/observability/scripts/contracts/check_trace_golden.py"
if [ "${ATLAS_E2E_ENABLE_OTEL:-0}" = "1" ]; then
  python3 "$ROOT/ops/observability/scripts/contracts/check_trace_coverage.py"
fi
python3 "$ROOT/ops/observability/scripts/contracts/extract_trace_exemplars.py"
...
root=Path('.')
contract=json.loads((root/'ops/observability/contract/metrics-contract.json').read_text())
tier0=[n for n,s in contract.get('required_metric_specs',{}).items() if s.get('criticality')=='tier-0']
text=(root/'artifacts/ops/observability/metrics.prom').read_text(encoding='utf-8',errors='replace')
...
ops=root/'artifacts/ops/observability'
...
metrics_schema=json.loads((root/'ops/observability/contract/metrics-contract.json').read_text()).get('schema_version')
trace_schema=json.loads((root/'docs/contracts/TRACE_SPANS.json').read_text()).get('schema_version')
logs_schema=json.loads((root/'ops/observability/contract/logs-fields-contract.json').read_text()).get('schema_version')
payload={
 'git_sha':sha,
