...
out="$ROOT/artifacts/ops/observability"
mkdir -p "$out"
printf '%s\n' '{"spans":[{"name":"request_root","request_id":"abc"}]}' > "$out/traces.snapshot.log"
printf '%s\n' '{"trace_id":"abc"}' > "$out/traces.exemplars.log"
if ATLAS_E2E_ENABLE_OTEL=1 python3 "$ROOT/ops/observability/scripts/contracts/check_trace_coverage.py" >/dev/null 2>&1; then
