...
# stability: internal
# called-by: ops/observability/tests/test_contracts.sh
import json,re,sys
from pathlib import Path
root=Path(__file__).resolve().parents[4]
contract=json.loads((root/'ops/observability/contract/metrics-contract.json').read_text(encoding='utf-8'))
allowed=set(contract.get('required_metric_specs',{}).keys())
dash=json.loads((root/'ops/observability/grafana/atlas-observability-dashboard.json').read_text(encoding='utf-8'))
exprs=[]
