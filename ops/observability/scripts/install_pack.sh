...
case "$PROFILE" in
  local-compose)
    ...
    if [ "$OFFLINE_MODE" = "1" ]; then
      ensure_offline_images
      $cmd -f "${REPO_ROOT}/ops/observability/pack/compose/docker-compose.yml" up -d --pull never
    else
      $cmd -f "${REPO_ROOT}/ops/observability/pack/compose/docker-compose.yml" up -d
    fi
    ;;
  kind | cluster)
    ops_kubectl apply -f "${REPO_ROOT}/ops/observability/pack/k8s/namespace.yaml"
    ops_kubectl apply -f "${REPO_ROOT}/ops/observability/pack/k8s/rbac.yaml"
    if [ "$OFFLINE_MODE" = "1" ]; then
      ensure_offline_images
      kind_load_offline_images
    fi
    ops_kubectl apply -f "${REPO_ROOT}/ops/observability/pack/k8s/prometheus-config.yaml"
    if [ "$STORAGE_MODE" = "persistent" ]; then
      ops_kubectl apply -f "${REPO_ROOT}/ops/observability/pack/k8s/prometheus-pvc.yaml"
      ops_kubectl apply -f "${REPO_ROOT}/ops/observability/pack/k8s/grafana-pvc.yaml"
    fi
    ops_kubectl apply -f "${REPO_ROOT}/ops/observability/pack/k8s/prometheus.yaml"
    ops_kubectl apply -f "${REPO_ROOT}/ops/observability/pack/k8s/grafana-config.yaml"
    ops_kubectl apply -f "${REPO_ROOT}/ops/observability/pack/k8s/grafana.yaml"
    kubectl -n "$OBS_NS" create configmap atlas-observability-otel-config \
      --from-file=config.yaml="${REPO_ROOT}/ops/observability/pack/otel/config.yaml" \
      --dry-run=client -o yaml | kubectl apply -f -
    ops_kubectl apply -f "${REPO_ROOT}/ops/observability/pack/k8s/otel.yaml"
    if ops_kubectl api-resources | grep -q "^prometheusrules"; then
      ops_kubectl -n "$OBS_NS" apply -f "${REPO_ROOT}/ops/observability/alerts/atlas-alert-rules.yaml"
    else
