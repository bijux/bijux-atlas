...
python3 "$ROOT/packages/bijux-atlas-scripts/src/bijux_atlas_scripts/layout_checks/check_tool_versions.py" k6 >/dev/null
...
RUN_REF="${RUN_ID:-$(cat "$ROOT/artifacts/evidence/latest-run-id.txt" 2>/dev/null || echo manual)}"
EVIDENCE_RAW="$ROOT/artifacts/evidence/perf/$RUN_REF/raw"
