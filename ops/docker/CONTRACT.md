...
- `docker.dir.no_contracts_subdir` (static, Pure): ops/docker/contracts subdirectory is forbidden
- `docker.dir.dockerfiles_location` (static, Pure): Dockerfiles must be under ops/docker/images/**
...
Tests:
- `docker.dockerfiles.under_images_only` (static, Pure): Dockerfiles are only under ops/docker/images/**
...
Tests:
- `docker.from.allowlisted_base_images` (static, Pure): FROM images are declared in ops/docker/bases.lock

### DOCKER-016 base image lock digest

Tests:
- `docker.from.digest_matches_lock` (static, Pure): FROM image digests match ops/docker/bases.lock
...
Tests:
- `docker.images.smoke_manifest` (static, Pure): each Docker image is listed with a smoke command in ops/docker/images.manifest.json

### DOCKER-034 image manifest schema

Tests:
- `docker.images.manifest_schema_valid` (static, Pure): ops/docker/images.manifest.json is schema-valid and non-empty
...
Tests:
- `docker.build_matrix.defined` (static, Pure): ops/docker/build-matrix.json covers every manifest image
...
Tests:
- `docker.registry.export_matches_generated` (static, Pure): ops/docker/docker.contracts.json matches generated registry output
...
Tests:
- `docker.exceptions.schema_valid` (static, Pure): ops/docker/exceptions.json uses the expected strict schema
