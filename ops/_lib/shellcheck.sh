...
if command -v shellcheck >/dev/null 2>&1; then
  xargs shellcheck --rcfile "$SHELLCHECK_RC" -x < "$LIST_FILE"
  exit 0
fi
...
if command -v docker >/dev/null 2>&1; then
  ...
  while IFS= read -r f; do
    rel="${f#"${ROOT}"/}"
    if ! docker run --rm -v "$ROOT:/mnt" koalaman/shellcheck:stable --rcfile "/mnt/configs/shellcheck/shellcheckrc" -x "/mnt/$rel"; then
      docker_fail=1
      break
    fi
  done < "$LIST_FILE"
