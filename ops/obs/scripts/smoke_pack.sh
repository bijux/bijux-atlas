...
"${REPO_ROOT}/ops/obs/scripts/snapshot_metrics.sh" "${REPO_ROOT}/artifacts/ops/obs"
"${REPO_ROOT}/ops/obs/scripts/snapshot_traces.sh" "${REPO_ROOT}/artifacts/ops/obs"

test -s "${REPO_ROOT}/artifacts/ops/obs/metrics.prom"
if [ "$PROFILE" != "local-compose" ]; then
  test -s "${REPO_ROOT}/artifacts/ops/obs/traces.snapshot.log"
fi
