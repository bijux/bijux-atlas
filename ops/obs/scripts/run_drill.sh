...
# stability: public
# called-by: ops/obs/tests/test_drills.sh, make ops-drill-runner
set -euo pipefail
...
MANIFEST="${REPO_ROOT}/ops/obs/drills/drills.json"
RESULT_SCHEMA="${REPO_ROOT}/ops/obs/drills/result.schema.json"
OUT_DIR="${REPO_ROOT}/artifacts/observability/drills"
OPS_OBS_DIR="${REPO_ROOT}/artifacts/ops/obs"
...
d=json.load(open('ops/obs/drills/drills.json'))
...
"${REPO_ROOT}/ops/obs/scripts/snapshot_metrics.sh" "$OPS_OBS_DIR"
...
root = pathlib.Path(".")
metrics = (root / "artifacts/ops/obs/metrics.prom").read_text(encoding="utf-8", errors="replace") if (root / "artifacts/ops/obs/metrics.prom").exists() else ""
traces = (root / "artifacts/ops/obs/traces.snapshot.log").read_text(encoding="utf-8", errors="replace").lower() if (root / "artifacts/ops/obs/traces.snapshot.log").exists() else ""
log_paths = sorted((root / "artifacts/ops/obs").glob("drill-*.logs.txt"))
...
result={
  ...
  "snapshot_paths":{
    "metrics":"artifacts/ops/obs/metrics.prom",
    "traces":"artifacts/ops/obs/traces.snapshot.log",
    "logs":os.environ["LOG_SNAPSHOT"]
  },
  "trace_ids":[],
  "expected_signals":json.loads(os.environ['EXPECTED_SIGNALS'])
}
trace_path=Path('artifacts/ops/obs/traces.snapshot.log')
...
import json,sys
from pathlib import Path
schema=json.load(open('ops/obs/drills/result.schema.json'))
