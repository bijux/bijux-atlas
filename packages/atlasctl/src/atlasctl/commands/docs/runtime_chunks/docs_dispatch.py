...
def run_docs_command(ctx: RunContext, ns: argparse.Namespace) -> int:
    ...
    if ns.docs_cmd in {"check", "validate"}:
        ...
        checks = [
            ...
            _check("docs-ops-entrypoints", "Validate ops docs entrypoint policy", ["python3", "packages/atlasctl/src/atlasctl/checks/domains/ops/contracts/check_ops_external_entrypoints.py"], "Reference only make targets and ops/run entrypoints in docs."),
    ...
    if ns.docs_cmd == "ops-entrypoints-check":
        return _run_simple_path(ctx, ns.report, "packages/atlasctl/src/atlasctl/checks/domains/ops/contracts/check_ops_external_entrypoints.py")
