...
def parse_mem_bytes(raw: str) -> int:
    ...
    match = re.match(r"^([0-9]*\.?[0-9]+)([KMGTP]i?)?B?$", used)
    if not match:
        return 0
...
for row in rows:
    lines.append(
        f"| {row['suite']} | {row['p50_ms']:.2f} | {row['p95_ms']:.2f} | {row['p99_ms']:.2f} | {row['fail_rate']:.4f} |"
    )
if baseline["runtime"]:
    lines.extend(
        [
...
print(f"wrote {out_dir / 'baseline.json'} and {out_dir / 'report.md'}")
