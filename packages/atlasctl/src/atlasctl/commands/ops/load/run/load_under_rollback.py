...
def main() -> int:
    ...
    def rollback_task() -> None:
        time.sleep(5)
        subprocess.run(["kubectl", "-n", ns, "rollout", "restart", f"deploy/{service}"], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        time.sleep(5)
        subprocess.run(["kubectl", "-n", ns, "rollout", "undo", f"deploy/{service}"], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        subprocess.run(
    ...
    if shutil.which("kubectl") and subprocess.run(["kubectl", "-n", ns, "get", "deploy", service], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode == 0:
        t = threading.Thread(target=rollback_task, daemon=True)
        t.start()

    subprocess.run(
        [str(root / "bin/atlasctl"), "ops", "load", "--report", "text", "run", "--suite", "load-under-rollback.json", "--out", str(out_dir)],
        check=True,
        cwd=str(root),
    )
    print("load-under-rollback complete")
    return 0
