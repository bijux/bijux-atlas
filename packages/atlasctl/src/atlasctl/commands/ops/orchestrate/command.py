...
def run_orchestrate_command(ctx: RunContext, ns: argparse.Namespace) -> int:
    ...
    if ns.cmd == "k8s":
        return _run_wrapped(ctx, OrchestrateSpec("k8s", ns.k8s_cmd, K8S_WRAPPER_ACTIONS[ns.k8s_cmd]), ns.report, dry_run=bool(getattr(ns, "dry_run", False)))
    if ns.cmd == "stack":
        if ns.stack_cmd == "reset":
            return _stack_reset(ctx, ns.report)
        return _run_wrapped(ctx, OrchestrateSpec("stack", ns.stack_cmd, STACK_WRAPPER_ACTIONS[ns.stack_cmd]), ns.report, dry_run=bool(getattr(ns, "dry_run", False)))
    if ns.cmd == "obs":
        return _run_wrapped(ctx, OrchestrateSpec("obs", ns.obs_cmd, OBS_WRAPPER_ACTIONS[ns.obs_cmd]), ns.report, dry_run=bool(getattr(ns, "dry_run", False)))
    if ns.cmd == "load":
        return _run_wrapped(ctx, OrchestrateSpec("load", ns.load_cmd, LOAD_WRAPPER_ACTIONS[ns.load_cmd]), ns.report, dry_run=bool(getattr(ns, "dry_run", False)))
    if ns.cmd == "e2e":
        return _run_wrapped(ctx, OrchestrateSpec("e2e", ns.e2e_cmd, E2E_WRAPPER_ACTIONS[ns.e2e_cmd]), ns.report, dry_run=bool(getattr(ns, "dry_run", False)))
    if ns.cmd == "datasets":
        return _run_wrapped(ctx, OrchestrateSpec("datasets", ns.datasets_cmd, DATASETS_WRAPPER_ACTIONS[ns.datasets_cmd]), ns.report, dry_run=bool(getattr(ns, "dry_run", False)))
