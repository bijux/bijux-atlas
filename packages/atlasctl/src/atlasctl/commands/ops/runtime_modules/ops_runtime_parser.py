...
def configure_ops_parser(sub: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    deploy_sub.add_parser("stack", help="prepare/deploy local stack substrate via workflows")
    deploy_sub.add_parser("atlas", help="deploy atlas workloads via helm workflow adapter")
    deploy_sub.add_parser("rollback", help="rollback/undeploy deployment")
    platform = ops_sub.add_parser("platform", help="ops platform lifecycle workflows")
    platform.add_argument("--report", choices=["text", "json"], default="text")
    platform_sub = platform.add_subparsers(dest="ops_platform_cmd", required=True)
    platform_sub.add_parser("up", help="bring up local platform (kind + stack prerequisites)")
    platform_sub.add_parser("down", help="tear down local platform (guarded)")
    ...
    env_sub.add_parser("doctor", help="doctor local ops environment against ops inventory toolchain")
    ...
    stack_reset = stack_sub.add_parser("reset", help="safe stack reset (kind only)")
    stack_reset.add_argument("--dry-run", action="store_true")
    stack_cleanup = stack_sub.add_parser("cleanup", help="prune owned stack artifacts only")
    stack_cleanup.add_argument("--dry-run", action="store_true")
    stack_sub.add_parser("smoke", help="run stack smoke suite")
    ...
    tests = ops_sub.add_parser("test", help="ops first-class test workflows")
    tests.add_argument("--report", choices=["text", "json"], default="text")
    tests_sub = tests.add_subparsers(dest="ops_test_cmd", required=True)
    tests_sub.add_parser("smoke", help="run smoke workflow")
    tests_sub.add_parser("e2e", help="run e2e workflow")
    tests_sub.add_parser("load", help="run load workflow")
    evidence = ops_sub.add_parser("evidence", help="ops evidence workflows")
    evidence.add_argument("--report", choices=["text", "json"], default="text")
    evidence_sub = evidence.add_subparsers(dest="ops_evidence_cmd", required=True)
    evidence_collect = evidence_sub.add_parser("collect", help="collect pods/logs/events evidence pointers")
    evidence_collect.add_argument("--namespace", default="atlas-e2e")
