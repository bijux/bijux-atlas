...
def configure_ci_parser(sub: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    scripts = ci_sub.add_parser("scripts", help="run scripts ci checks")
    scripts.add_argument("--json", action="store_true", help="emit JSON output")
    scripts.add_argument("--verbose", action="store_true", help="show underlying tool command output")
