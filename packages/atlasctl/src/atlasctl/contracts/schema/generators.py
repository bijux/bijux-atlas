...
def generate_chart_schema(repo_root: Path) -> list[str]:
    ...
    write_text_file(out, json.dumps(schema, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return []
...
def generate_contract_artifacts(repo_root: Path) -> list[str]:
    ...
    write_text_file(core_generated_dir / "mod.rs", "pub mod error_codes;\n", encoding="utf-8")
    ...
    core_rust.extend([f'    "{code}",' for code in error_codes])
    core_rust.append("];\n")
    write_text_file(core_rust_path, "\n".join(core_rust), encoding="utf-8")
    ...
    write_text_file(server_gen_dir / "mod.rs", 
    ...
    metric_lines.append("];\n")
    write_text_file(server_gen_dir / "metrics_contract.rs", "\n".join(metric_lines), encoding="utf-8")
    ...
    span_lines.append("];\n")
    write_text_file(server_gen_dir / "trace_spans_contract.rs", "\n".join(span_lines), encoding="utf-8")
