...
# Inputs: ops/_generated.example and docs/_generated ops artifacts.
...
expected = {
    "ops/_generated.example/.gitkeep",
...
unknown = sorted(
    ...
    for p in (ROOT / "ops/_generated.example").rglob("*")
    if p.is_file()
    and p.relative_to(ROOT).as_posix() != "ops/_generated.example/report.unified.json"
