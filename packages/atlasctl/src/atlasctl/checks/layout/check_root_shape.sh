...
# Inputs: packages/atlasctl/src/atlasctl/layout_checks/root_whitelist.json and root filesystem entries.
# Outputs: non-zero exit on missing required or unexpected root entries.
set -euo pipefail

ROOT="$(CDPATH= cd -- "$(dirname -- "$0")/../../.." && pwd)"
ALLOW_JSON="$ROOT/packages/atlasctl/src/atlasctl/layout_checks/root_whitelist.json"
