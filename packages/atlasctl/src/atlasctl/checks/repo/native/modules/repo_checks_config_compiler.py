...
_GEN_HEADER = "# GENERATED BY atlasctl config gen. DO NOT EDIT."
...
def check_config_generated_headers_and_checksums(repo_root: Path) -> tuple[int, list[str]]:
    ...
    if not root.exists():
        return 1, ["configs/_generated missing; run `atlasctl config gen`"]
    checksums = root / "checksums.json"
    if not checksums.exists():
        return 1, ["configs/_generated/checksums.json missing; run `atlasctl config gen`"]
