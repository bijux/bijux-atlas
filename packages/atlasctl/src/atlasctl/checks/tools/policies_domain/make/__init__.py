...
from ....tools.make_enforcement import (
    ...
    check_ci_workflows_call_only_delegating_make_targets,
    ...
    check_make_ops_product_no_tool_tokens,
    check_make_ops_product_atlasctl_only_delegation,
    check_make_ops_product_ownership_complete,
    check_make_removed_makefiles_unreferenced,
    ...
    check_ops_bypass_ledger_task_ids,
    check_ops_bypass_ledger_sorted,
    check_ops_bypass_ledger_domains,
    check_ops_allowlisted_literals_have_contract_tests,
    check_ops_bypass_entries_have_necessity_tests,
    check_policies_bypass_severity_levels,
    check_policies_bypass_replacement_mechanism,
    check_policies_bypass_severity_expiry_windows,
    check_policies_bypass_no_security_or_pinning,
    check_policies_bypass_no_unpinned_images,
    check_policies_bypass_no_network_guard,
    check_policies_bypass_no_direct_script_runs,
    check_policies_bypass_tokens_linked,
    check_policies_culprits_empty_on_release_tags,
    check_policies_culprits_nonincrease_on_main,
...
def check_policies_bypass_files_scoped(repo_root: Path) -> tuple[int, list[str]]:
    ...
    for base in scan_roots:
        ...
        for path in base.rglob("*"):
            ...
            if rel.startswith("configs/policy/") or rel.startswith("configs/docs/") or rel.startswith("configs/ops/") or rel.startswith("configs/schema/") or rel.startswith("configs/layout/") or rel.startswith("configs/make/") or rel.startswith("configs/repo/") or rel.startswith("configs/security/") or rel.startswith("ops/_artifacts/") or rel.startswith("ops/_lint/") or rel.startswith("ops/_meta/") or rel.startswith("ops/schema/") or rel.startswith("ops/vendor/layout-checks/") or rel.startswith("packages/atlasctl/src/atlasctl/checks/layout/makefiles/policies/") or rel.startswith("packages/atlasctl/src/atlasctl/checks/layout/ops/runtime/") or rel.startswith("packages/atlasctl/src/atlasctl/commands/ops/lint/policy/"):
                continue
...
def check_policies_bypass_hard_gate(repo_root: Path) -> tuple[int, list[str]]:
    ...
    if not enabled:
        enabled = str(os.environ.get("ATLASCTL_BYPASS_ZERO_MODE", "")).strip().lower() in {"1", "true", "yes", "on"}
    if not enabled:
        return 0, []
    current = int(collect_bypass_inventory(repo_root).get("entry_count", 0))
    if current == 0:
...
CHECKS: tuple[CheckDef, ...] = (
    ...
    CheckDef("make.ops_product_no_tool_tokens", "make", "ops.mk/product.mk must not contain raw tool tokens", 1000, check_make_ops_product_no_tool_tokens, fix_hint="Keep ops.mk and product.mk as atlasctl-only wrappers (no bash/python/kubectl/helm/docker recipe tokens)."),
    CheckDef("make.ops_product_atlasctl_only_delegation", "make", "ops.mk and product.mk must delegate only to matching atlasctl groups", 1000, check_make_ops_product_atlasctl_only_delegation, fix_hint="Use ./bin/atlasctl ops ... in ops.mk and ./bin/atlasctl product ... in product.mk."),
    CheckDef("make.ops_product_ownership_complete", "make", "ownership metadata must cover ops/product targets", 1000, check_make_ops_product_ownership_complete, fix_hint="Add ownership metadata entries for all public ops/product make targets."),
    CheckDef("make.removed_makefiles_unreferenced", "make", "retired makefiles must not be referenced", 1000, check_make_removed_makefiles_unreferenced, fix_hint="Remove references to retired makefiles or update configs/make/retired-makefiles.json intentionally."),
    CheckDef("make.ci_workflows_delegating_targets_only", "make", "CI workflows must call only make targets that delegate to atlasctl", 1000, check_ci_workflows_call_only_delegating_make_targets, fix_hint="Use canonical delegating make targets in CI workflows."),
    ...
    CheckDef("make.help_determinism", "make", "ensure deterministic make help output", 4000, check_make_help, fix_hint="Regenerate and normalize make help output.", effects=("fs_read", "subprocess")),
    ...
    CheckDef("policies.adjectives_repo_clean", "policies", "forbid forbidden adjectives across tracked files", 800, check_policies_adjectives_repo_clean, fix_hint="Replace forbidden adjectives with neutral wording.", tags=("repo",), effects=("fs_read", "subprocess")),
    ...
    CheckDef("policies.bypass_entry_matches_nothing", "policies", "fail if wildcard bypass entry matches nothing", 4000, check_policies_bypass_entry_matches_nothing, fix_hint="Tighten or remove stale wildcard bypass entries.", tags=("repo",)),
    CheckDef("policies.bypass_entry_matches_too_broad", "policies", "fail if wildcard bypass entry matches too broadly", 4000, check_policies_bypass_entry_matches_too_broad, fix_hint="Narrow wildcard bypass scope to intended paths only.", tags=("repo",)),
    ...
    CheckDef("policies.bypass_replacement_mechanism", "policies", "require replacement_mechanism on bypass entries", 800, check_policies_bypass_replacement_mechanism, fix_hint="Add replacement_mechanism to bypass entries.", tags=("repo",)),
    CheckDef("policies.bypass_severity_expiry_windows", "policies", "require tighter expiry windows for P0/P1 bypasses", 800, check_policies_bypass_severity_expiry_windows, fix_hint="Reduce expiry horizon for P0/P1 bypasses.", tags=("repo",)),
    CheckDef("policies.bypass_no_security_or_pinning", "policies", "forbid bypasses in security and image pinning policy areas", 800, check_policies_bypass_no_security_or_pinning, fix_hint="Remove bypasses for security/image pinning policy areas.", tags=("repo", "required")),
    CheckDef("policies.bypass_no_unpinned_images", "policies", "forbid bypasses for unpinned images policy", 800, check_policies_bypass_no_unpinned_images, fix_hint="Remove unpinned-image bypasses and pin images.", tags=("repo", "required")),
    CheckDef("policies.bypass_no_network_guard", "policies", "forbid bypasses for network guard policy", 800, check_policies_bypass_no_network_guard, fix_hint="Remove network-guard bypasses and use explicit network policy flags.", tags=("repo", "required")),
    CheckDef("policies.bypass_no_direct_script_runs", "policies", "forbid bypasses for direct script runs policy", 800, check_policies_bypass_no_direct_script_runs, fix_hint="Remove direct-script-run bypasses and use atlasctl commands.", tags=("repo", "required")),
    CheckDef("policies.bypass_tokens_linked", "policies", "forbid ALLOWLIST/TEMP/HACK tokens without linked bypass ids", 800, check_policies_bypass_tokens_linked, fix_hint="Link tokens to OPS-BYPASS-* or ATLAS-EXC-* identifiers.", tags=("repo",)),
    CheckDef("policies.culprits_empty_on_release_tags", "policies", "require zero policy culprits on release tags", 800, check_policies_culprits_empty_on_release_tags, fix_hint="Release tags must ship without bypass culprits.", tags=("repo", "ci")),
    CheckDef("policies.culprits_nonincrease_on_main", "policies", "forbid bypass count increases on main", 800, check_policies_culprits_nonincrease_on_main, fix_hint="Reduce or approve bypass growth before merging to main.", tags=("repo", "ci")),
