...
def test_schema_files_are_referenced_by_docs_or_code() -> None:
    ...
    corpus = []
    for path in [ROOT / "docs", ROOT / "packages/bijux-atlas-scripts/src", ROOT / "makefiles"]:
        for f in path.rglob("*"):
            if f.is_file():
                try:
...
def test_ops_generated_runtime_dir_not_required_by_tooling() -> None:
    command_roots = [
        ROOT / "packages/bijux-atlas-scripts/src/bijux_atlas_scripts/cli.py",
        ROOT / "packages/bijux-atlas-scripts/src/bijux_atlas_scripts/orchestrate/command.py",
        ROOT / "packages/bijux-atlas-scripts/src/bijux_atlas_scripts/report/command.py",
        ROOT / "packages/bijux-atlas-scripts/src/bijux_atlas_scripts/gates/command.py",
        ROOT / "packages/bijux-atlas-scripts/src/bijux_atlas_scripts/docs/command.py",
        ROOT / "packages/bijux-atlas-scripts/src/bijux_atlas_scripts/configs/command.py",
        ROOT / "packages/bijux-atlas-scripts/src/bijux_atlas_scripts/ops/command.py",
    ]
...
def test_no_shell_true_subprocess_usage() -> None:
    offenders = []
    for path in sorted((ROOT / "packages/bijux-atlas-scripts/src").rglob("*.py")):
        text = path.read_text(encoding="utf-8")
...
def test_python_dependencies_are_pinned_in_lock() -> None:
    lines = (ROOT / "packages/bijux-atlas-scripts/requirements.lock.txt").read_text(encoding="utf-8").splitlines()
...
def test_no_print_outside_cli_and_command_modules() -> None:
    offenders = []
    enforce_roots = (
        ROOT / "packages/bijux-atlas-scripts/src/bijux_atlas_scripts/core",
        ROOT / "packages/bijux-atlas-scripts/src/bijux_atlas_scripts/registry",
        ROOT / "packages/bijux-atlas-scripts/src/bijux_atlas_scripts/report",
    )
